...
public class ToDoListPresenter extends BasePresenter<ToDoListFragmentContract.View> implements ToDoListFragmentContract.Presenter {
    ...
    public void getToDoList(int page, boolean isFresh) {
        if (mView != null) {
            ...
            HttpsUtil.request(HttpsUtil.createApi(GankApplication.getContext(), AppConfig.BASE_URL, ApiService.class).getToDoList(page, map), new HttpsUtil.IResponseListener<ToDoListBean>() {
                @Override
    ...
    public void deleteTodo(int id) {
        if (mView != null) {
            ...
            HttpsUtil.request(HttpsUtil.createApi(GankApplication.getContext(), AppConfig.BASE_URL, ApiService.class).deleteTodo(id), new HttpsUtil.IResponseListener<AddToDoBean>() {
                @Override
    ...
    public void updateToDoStatus(int id, int status) {
        if (mView != null) {
            ...
            HttpsUtil.request(HttpsUtil.createApi(GankApplication.getContext(), AppConfig.BASE_URL, ApiService.class).updateToDoStatus(id, status), new HttpsUtil.IResponseListener<AddToDoBean>() {
                @Override
                public void onSuccess(AddToDoBean data) {
                    if (data.getErrorCode() == 0) {
